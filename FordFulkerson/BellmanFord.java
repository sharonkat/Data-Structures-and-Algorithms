public class BellmanFord{
    ...
    BellmanFord(WGraph g, int source) throws BellmanFordException{
    	...
    	// The second part of the if statement is checking:
    	// if d[v] > d[u] + w(u,v)
    	// We relax all the edges and say it is not a negative-weight loop
    	...
    	// After relaxing all the edges, we check again to see if the edges can be relaxed again:
    	// Is it true that d[v] > d[u] + w(u,v)? If yes then oh shoot we have a negative-weight cycle!
    	// So we want to throw an exception :)
    	for (Edge e : g.getEdges()) {
    		if ((distances[e.nodes[0]] != Integer.MAX_VALUE)
    		&& ((distances[e.nodes[0]] + e.weight) < distances[e.nodes[1]])) {
    			throw new NegativeWeightException("This graph contains negaative-weight cycles!");
    		}
    	}
    }
    ...
    public int[] shortestPath(int destination) throws BellmanFordException{
        ...
        int[] adjacency = new int[distances.length];
        int size = 1;
        adjacency[0] = destination;
        ...
        while (curr != source) {
        	...
        	// Build our array with "adjacent" nodes
        	adjacency[size] = curr;
        	size++;
        }
        
        int[] path = new int[size];
        // visit nodes based on adjacency array
        for (int i=0; i<size; i++) {
        	path[i] = adjacency[size-(i+1)];
        }
        return path;
    }
